...
// ==========================================
// REFERRAL AUTO-SPEND & JADWAL KOMISI
// ==========================================

/**
 * Otomatisasi pengeluaran (auto-spend) dari penjualan referral:
 * Setiap transaksi yang berstatus 'COMPLETED' dan memiliki pengajak/referral (source: 'Referral' atau field referrer terisi),
 * sistem akan otomatis membuat catatan pengeluaran (Expense) ber-kategori 'Komisi Freelancer'.
 *
 * Aturan Jatuh Tempo Pembayaran:
 * Komisi penjualan bulan X dijadwalkan dibayarkan pada tanggal 1 di bulan X+1.
 * Contoh: Penjualan September 2026 -> Dijadwalkan dibayarkan 1 Oktober 2026.
 */
function syncReferralAutoSpend(tx: Transaction, action: 'ADD' | 'UPDATE' | 'DELETE') {
  if (action === 'DELETE' || tx.status !== 'COMPLETED') {
    expenses = expenses.filter(e => e.transactionId !== tx.id);
    return;
  }

  const isReferral = tx.source === 'Referral' || Boolean(tx.referrer && tx.referrer.trim());
  if (!isReferral) {
    expenses = expenses.filter(e => e.transactionId !== tx.id);
    return;
  }

  // Cari mitra freelancer yang cocok berdasarkan kode unik referral atau nama
  const refStr = (tx.referrer || '').trim();
  const matched = freelancers.find(f => {
    const c = f.code.toUpperCase();
    const n = f.name.toUpperCase();
    const r = refStr.toUpperCase();
    return r && (r === c || r === n || r.includes(c));
  });

  const fCode = matched ? matched.code : (refStr || 'REFERRAL');
  const fName = matched ? matched.name : (refStr || 'Mitra Referral');
  const rate = matched?.commissionRate ?? 10;
  const isFixed = matched?.commissionType === 'FIXED';
  const commissionAmount = isFixed ? rate : Math.round(((tx.amount || 0) * rate) / 100);

  if (commissionAmount <= 0) return;

  const schedule = getReferralPayoutSchedule(tx.date);

  const existingIdx = expenses.findIndex(e => e.transactionId === tx.id);
  const autoExpense: Expense = {
    id: existingIdx >= 0 ? expenses[existingIdx].id : `exp-ref-${tx.id}`,
    title: `Komisi Referral: ${fName} (${tx.productName || 'Produk Valiyo'})`,
    category: 'Komisi Freelancer',
    amount: commissionAmount,
    date: tx.date || new Date().toISOString().split('T')[0],
    recipient: `${fName} (${fCode})`,
    paymentMethod: 'Transfer Bank',
    notes: `Auto-spend komisi penjualan referral ${rate}% dari ${tx.productName || 'Produk'} (${formatRupiah(tx.amount)}). Jadwal transfer: ${schedule.scheduledDateLabel}.`,
    createdAt: existingIdx >= 0 ? expenses[existingIdx].createdAt : new Date().toISOString(),
    transactionId: tx.id,
    freelancerId: matched?.id,
    freelancerCode: fCode,
    period: schedule.period,
    scheduledPaymentDate: schedule.scheduledDateLabel,
    payoutStatus: existingIdx >= 0 ? expenses[existingIdx].payoutStatus || 'SCHEDULED' : 'SCHEDULED',
    paidAt: existingIdx >= 0 ? expenses[existingIdx].paidAt : undefined,
    isAutoGenerated: true
  };

  if (existingIdx >= 0) {
    expenses[existingIdx] = autoExpense;
  } else {
...
/**
 * Menghitung rekap komisi referral per bulan yang harus dibayarkan.
 * Mengelompokkan berdasarkan periode bulan transaksi (contoh: September 2026 -> jadwal bayar 1 Oktober 2026),
 * serta rincian komisi per mitra freelancer.
 */
function calculateMonthlyReferralRecaps(): MonthlyReferralRecap[] {
  const referralTxs = transactions.filter(
    t => t.status === 'COMPLETED' && (t.source === 'Referral' || Boolean(t.referrer && t.referrer.trim()))
  );

  const referralExpenses = expenses.filter(
    e => e.category === 'Komisi Freelancer' || Boolean(e.freelancerCode) || Boolean(e.transactionId)
  );
  ...
  const periodMap = new Map<
    string,
    {
      ...
      dueDateIso: string;
      freelancerMap: Map<string, FreelancerPayoutSummary>;
    }
  >();

  const getOrCreatePeriod = (dateStr?: string, explicitPeriod?: string) => {
    const sched = getReferralPayoutSchedule(dateStr);
    const pKey = explicitPeriod || sched.period;
    if (!periodMap.has(pKey)) {
      periodMap.set(pKey, {
        period: pKey,
        scheduledPayoutDate: sched.scheduledDateLabel,
        dueDateIso: sched.scheduledDateIso,
        freelancerMap: new Map<string, FreelancerPayoutSummary>()
      });
    }
    return periodMap.get(pKey)!;
  };

  // 1. Akumulasi dari transaksi referral
  for (const tx of referralTxs) {
    const sched = getReferralPayoutSchedule(tx.date);
    const pEntry = getOrCreatePeriod(tx.date, sched.period);
    ...
    const fCode = matched ? matched.code : (refStr || 'REFERRAL');
    const fName = matched ? matched.name : (refStr || 'Mitra Referral');
    const fPhone = matched?.phone;
    const fId = matched?.id;
    const rate = matched?.commissionRate ?? 10;
    const commType = matched?.commissionType ?? 'PERCENTAGE';
    const commAmt = commType === 'FIXED' ? rate : Math.round(((tx.amount || 0) * rate) / 100);

    if (!pEntry.freelancerMap.has(fCode)) {
      pEntry.freelancerMap.set(fCode, {
        freelancerId: fId,
        ...
        scheduledDateLabel: sched.scheduledDateLabel,
        paidAt: undefined,
        transactionIds: [],
        expenseIds: []
      });
    ...
    const item = pEntry.freelancerMap.get(fCode)!;
    item.transactionCount += 1;
    item.totalSales += tx.amount || 0;
    item.totalCommission += commAmt;
    if (!item.transactionIds.includes(tx.id)) {
      item.transactionIds.push(tx.id);
    }
  }

  // 2. Hubungkan dengan catatan pengeluaran
  for (const exp of referralExpenses) {
    const sched = getReferralPayoutSchedule(exp.date);
    const pKey = exp.period || sched.period;
    const pEntry = getOrCreatePeriod(exp.date, pKey);

    const fCode =
      exp.freelancerCode ||
      (exp.recipient ? exp.recipient.split('(')[1]?.replace(')', '').trim() : '') ||
      'REFERRAL';

    if (!pEntry.freelancerMap.has(fCode)) {
      pEntry.freelancerMap.set(fCode, {
        freelancerId: exp.freelancerId,
        freelancerName: exp.recipient ? exp.recipient.split('(')[0].trim() : 'Mitra Referral',
        freelancerCode: fCode,
        phone: undefined,
        transactionCount: exp.transactionId ? 1 : 0,
        totalSales: 0,
        commissionRate: 10,
        commissionType: 'PERCENTAGE',
        totalCommission: exp.amount,
        payoutStatus: exp.payoutStatus || 'SCHEDULED',
        scheduledDateLabel: exp.scheduledPaymentDate || sched.scheduledDateLabel,
        paidAt: exp.paidAt,
        transactionIds: exp.transactionId ? [exp.transactionId] : [],
        expenseIds: [exp.id]
      });
    } else {
      const item = pEntry.freelancerMap.get(fCode)!;
      if (!item.expenseIds.includes(exp.id)) {
        item.expenseIds.push(exp.id);
      }
      if (exp.payoutStatus === 'PAID') {
        item.payoutStatus = 'PAID';
        item.paidAt = exp.paidAt;
      }
    }
  }

  // 3. Bangun array ringkasan rekap bulanan
  const recaps: MonthlyReferralRecap[] = [];
  for (const pEntry of periodMap.values()) {
    const fList = Array.from(pEntry.freelancerMap.values());
    const totalReferralSales = fList.reduce((sum, f) => sum + f.totalSales, 0);
    const totalTransactions = fList.reduce((sum, f) => sum + f.transactionCount, 0);
    const totalCommission = fList.reduce((sum, f) => sum + f.totalCommission, 0);

    let totalPaid = 0;
    let totalPending = 0;
    for (const f of fList) {
      if (f.payoutStatus === 'PAID') {
        totalPaid += f.totalCommission;
      } else {
        totalPending += f.totalCommission;
      }
    }

    let status: 'PENDING' | 'PAID' | 'PARTIAL' = 'PENDING';
    if (totalCommission > 0 && totalPaid >= totalCommission) {
      status = 'PAID';
    } else if (totalPaid > 0) {
      status = 'PARTIAL';
    }
    ...
    recaps.push({
      ...
      scheduledPayoutDate: pEntry.scheduledPayoutDate,
      dueDateIso: pEntry.dueDateIso,
      totalReferralSales,
      totalTransactions,
      totalCommission,
      totalPaid,
      totalPending,
      freelancersCount: fList.length,
      status,
      freelancers: fList
    });
  }

  recaps.sort((a, b) => (b.dueDateIso || '').localeCompare(a.dueDateIso || ''));
  return recaps;
}
...
function getSystemState() {
  ...
  const monthlyReferralRecaps = calculateMonthlyReferralRecaps();
...
app.get('/api/state', (req, res) => {
  try {
    ...
    res.json({
      ...
      employees,
      freelancers,
      monthlyReferralRecaps: derived.monthlyReferralRecaps,
      products,
      tasks: sortedTasks,
      b2bDeals,
      goals,
...
// ==========================================
// REKAP REFERRAL BULANAN & SETTLEMENT PAYOUTS
// ==========================================
app.get('/api/referrals/recap', (req, res) => {
  try {
    const recaps = calculateMonthlyReferralRecaps();
    res.json({
      recaps,
      totalPendingAllTime: recaps.reduce((sum, r) => sum + r.totalPending, 0),
      totalPaidAllTime: recaps.reduce((sum, r) => sum + r.totalPaid, 0)
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message || 'Gagal memuat rekap referral' });
  }
});

// Settle / Tandai Pembayaran Komisi Sudah Dibayarkan
app.post('/api/referrals/payouts/settle', (req, res) => {
  const { period, freelancerCode, paymentDate, notes } = req.body;
  const nowIso = paymentDate || new Date().toISOString().split('T')[0];
  let updatedCount = 0;
  ...
  expenses.forEach(e => {
    const matchesPeriod =
      !period ||
      e.period === period ||
      (e.scheduledPaymentDate && e.scheduledPaymentDate.includes(period));
    const matchesFreelancer =
      !freelancerCode ||
      e.freelancerCode === freelancerCode ||
      (e.recipient && e.recipient.includes(freelancerCode));

    if (matchesPeriod && matchesFreelancer && (e.category === 'Komisi Freelancer' || Boolean(e.transactionId))) {
      e.payoutStatus = 'PAID';
      e.paidAt = nowIso;
      if (notes) {
        e.notes = (e.notes ? e.notes + ' | ' : '') + notes;
      }
      updatedCount++;
    }
  });
  ...
  res.json({
    success: true,
    updatedCount,
    monthlyReferralRecaps: calculateMonthlyReferralRecaps(),
    systemState: getSystemState(),
    message: `Berhasil mencatat pembayaran komisi (${updatedCount} catatan) status Lunas.`
  });
});

// Kembalikan status komisi ke Terjadwal
app.post('/api/referrals/payouts/revert', (req, res) => {
  const { period, freelancerCode } = req.body;
  let revertedCount = 0;
  ...
  res.json({
    ...
    monthlyReferralRecaps: calculateMonthlyReferralRecaps(),
    systemState: getSystemState(),
    message: `Status komisi referral dikembalikan ke Terjadwal.`
  });
});
...
app.post('/api/demo/reset', (req, res) => {
  ...
  isDemoMode = true;
  for (const tx of transactions) {
    if (tx.status === 'COMPLETED' && (tx.source === 'Referral' || tx.referrer)) {
      syncReferralAutoSpend(tx, 'ADD');
    }
  }
  recalculateRevenueAndMonthlyHistory();
  persistState();
...
function applyTransactionImpact(tx: Transaction, isAddition: boolean) {
  if (isAddition) {
    ...
    syncReferralAutoSpend(tx, 'DELETE');
  }
  recalculateRevenueAndMonthlyHistory();
}
...
app.post('/api/transactions/clear', (req, res) => {
  // Bersihkan juga auto-spend referral terkait transaksi
  expenses = expenses.filter(e => !e.transactionId && !e.isAutoGenerated);
  transactions = [];
  recalculateRevenueAndMonthlyHistory();
